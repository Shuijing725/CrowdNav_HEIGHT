...
class CrowdSim3DTB(CrowdSimVarNum):
    def __init__(self):
        ...
        # Initialize velocity history queues (length 3) for jackal sim2real
        self.v_queue = deque([0.0, 0.0, 0.0], maxlen=3)
        self.w_queue = deque([0.0, 0.0, 0.0], maxlen=3)
    ...
    def configure(self, config):
        ...
        self.border_size = self.arena_size + self.config.sim.human_pos_noise_range - 0.5

        # for debugging: replaying recorded real human trajectories
        self.replay_spatial_edges = None
        self.replay_step = 0
        self.use_replay_spatial_edges = self.config.sim2real.replay_human_traj
    ...
    def set_action_space(self):
        if self.config.env.action_space == 'continuous':
            ...
            # turtlebot can turn in place so we can use a larger rotational acceleration
            # use 0.5 rad/s^2 as max angular acceleration & decceleration to reduce slippage and wheel damage to jackal
    ...
    def loadArena(self):
        ...
        if self.config.sim.borders:
            if self.config.env.scenario == 'csl_workspace':
                if self.config.env.csl_workspace_type == 'gdc':
                    wall_pos = [0, 0, -0.2]
                else:
                    wall_pos = [0, 0, -0.02]
    ...
    def ray_test(self, camera_fov=False):
        ...
        if camera_fov:
            self.rayFrom = self.rayFrom + np.array([self.config.camera.lower_sensor_range * np.cos(self.robot.theta + ray_angles),
                          self.config.camera.lower_sensor_range * np.sin(self.robot.theta + ray_angles),
                          np.zeros([ray_num])]).T # [3, ray_num]
            # end xyz pos of all rays
            # robot frame
            rayTo = np.array([self.config.camera.upper_sensor_range * np.cos(self.robot.theta + ray_angles),
                              self.config.camera.upper_sensor_range * np.sin(self.robot.theta + ray_angles),
    ...
    # if camera_fov is True, use the camera FOV to perform ray test (to get human detections, used in self.get_num_human_in_fov())
    # otherwise, use the lidar with 360 FOV to perform ray test (to get raw lidar pc including obstacles, as ob['point_clouds'])
    def ray_test_check_human_visibility(self):
        """
        perform a lidar ray test on all humans and obstacles in the scene
        save the current range readings in self.closest_hit_dist, the object IDs in self.closest_hit_id, the uids of all detected humans in self.visible_human_uids
        ...
        # reduce a few rays on the edge of camera FoV
        reduce_ray_num = 12
        ray_num = self.cam_ray_num - reduce_ray_num
        ray_angles = self.cam_ray_angles[int(reduce_ray_num/2) : int(-reduce_ray_num/2)]
        # ray_num = self.cam_ray_num
        # ray_angles = self.cam_ray_angles
        ...
        # uids of human that can be detected by robot's Lidar
        self.cam_visible_human_uids = set()

        # parse the results and visualize lidar rays

        for i in range(ray_num):
            if results[i][0] in self.used_human_uids:
                self.cam_visible_human_uids.add(results[i][0]) # set can avoid duplicates
    ...
    def render_scene(self):
        ...
        elif self.config.env.scenario == 'csl_workspace' and self.config.env.csl_workspace_type == 'gdc':
            view_matrix = p.computeViewMatrix(
                cameraEyePosition=[-5, 5, 20], # x, y, height
    ...
    def create_scenario(self, phase='train', test_case=None):
        ...
        if self.physicsClientId < 0:
            ...
            if self.config.action_space.kinematics == "jackal":
                self.robot.uid = p.loadURDF("crowd_sim/pybullet/media/jackal/jackal.urdf", [0, 0, 0])
            else:
        ...
        if self.add_static_obs:
            ...
            for i in range(self.obs_num):
                ...
                elif hasattr(self.config.sim, 'obs_offset'):
                    center_z = self.config.sim.obs_offset
                else:
                    center_z = 0.
    ...
    def reset(self, phase='train', test_case=None):
        ...
        # Load spatial_edges log automatically if configured
        if hasattr(self.config.sim2real,
                   'replay_spatial_edges_path') and self.config.sim2real.replay_human_traj:
            try:
                data = np.load(self.config.sim2real.replay_spatial_edges_path)
                self.replay_spatial_edges = data['spatial_edges']
                self.use_replay_spatial_edges = True
                self.replay_step = 0
                print(
                    f"[Replay] Loaded spatial edges from {self.config.sim2real.replay_spatial_edges_path}, shape: {self.replay_spatial_edges.shape}")
            except Exception as e:
                print(f"[Replay] Failed to load spatial edges: {e}")
                self.use_replay_spatial_edges = False
        ...
        # Clear queues at the start of each episode
        self.v_queue = deque([0.0, 0.0, 0.0], maxlen=3)
        self.w_queue = deque([0.0, 0.0, 0.0], maxlen=3)

        self.replay_step = 0  # reset replay pointer
    ...
    def tb2_dynamics_noise(self, left, right):
    ...
    # given desired v and w in sim,
    # return the actual v and w approximated from real trajectories
    def jackal_dynamics_noise(self, v, w):
        # # no queue
        # if self.step_counter < 2:
        #     return 0, 0
        # else:
        ...
        # queue: Append current commands to queues
        self.v_queue.append(v)
        self.w_queue.append(w)

        # Return values from 3 steps ago (front of the queue)
        delayed_v = self.v_queue[0]
        delayed_w = self.w_queue[0]
        return delayed_v, delayed_w
    ...
    def step(self, action, update=True):
        ...
        # for i, h_action in enumerate(human_actions):
        #     h_norm = np.hypot(h_action.vx, h_action.vy)
        #     print(f'Human {i}: norm = {h_norm}')

        # compute reward and episode info
        reward, done, episode_info = self.calc_reward(action)

        if self.load_act:
            if self.step_counter >= len(self.episodeRecoder.v_list) - 2:
                done = True
        ...
        if self.config.env.action_space == 'continuous':
            ...
            if self.load_act:
                self.desiredVelocity[0] = self.episodeRecoder.v_list[self.step_counter]
                self.desiredVelocity[1] = self.episodeRecoder.delta_theta_list[self.step_counter]
            else:
            ...
            if self.config.action_space.kinematics == 'turtlebot':
                wheel_track_width = 0.23
                wheel_radius = 0.035
            else:
                wheel_track_width = 0.365
                wheel_radius = 0.097
            left = (2. * self.desiredVelocity[0] - wheel_track_width * self.desiredVelocity[1]) / (2. * wheel_radius)
            right = (2. * self.desiredVelocity[0] + wheel_track_width * self.desiredVelocity[1]) / (2. * wheel_radius)
        # print('commanded v and w', self.desiredVelocity)

        # simulate the turtlebot dynamics

        if self.config.action_space.kinematics == 'turtlebot':
            left, right = self.tb2_dynamics_noise(left, right)
            ...
            # use pybullet to step the robot
            # 1 front_left_wheel
            # 2 front_right_wheel
            # 3 rear_left_wheel
            # 4 rear_right_wheel
            # p.setJointMotorControl2(self.robot.uid, 1, p.VELOCITY_CONTROL, targetVelocity=left, force=10)
            # p.setJointMotorControl2(self.robot.uid, 2, p.VELOCITY_CONTROL, targetVelocity=right, force=10)
            # p.setJointMotorControl2(self.robot.uid, 3, p.VELOCITY_CONTROL, targetVelocity=left, force=10)
            # p.setJointMotorControl2(self.robot.uid, 4, p.VELOCITY_CONTROL, targetVelocity=right, force=10)

            # overwrites pybullet dynamics, differential drive
            v_smooth, w_smooth = self.jackal_dynamics_noise(self.desiredVelocity[0], self.desiredVelocity[1])
            # print('actual v and w', v_smooth, w_smooth)
            epsilon = 0.0001
            # R is the radius of the turning arc
            if abs(w_smooth * self.time_step) < epsilon:
                R = 0
            else:
                R = v_smooth / w_smooth

            robot_yaw = (self.robot.theta + w_smooth * self.time_step) % (2 * np.pi)
            # robot_px = self.robot.px - R * np.sin(self.robot.theta) + R * np.sin(robot_yaw)
            # robot_py = self.robot.py + R * np.cos(self.robot.theta) - R * np.cos(robot_yaw)
            robot_px = self.robot.px + v_smooth * np.cos(robot_yaw) * self.time_step
            robot_py = self.robot.py + v_smooth * np.sin(robot_yaw) * self.time_step
            # print('px, py before', self.robot.px, self.robot.py)
            # print('px, py after', robot_px, robot_py)

            robot_vx = v_smooth * np.cos(self.robot.theta)
            robot_vy = v_smooth * np.sin(self.robot.theta)
            robot_wz = w_smooth
            pybullet_pos, _ = p.getBasePositionAndOrientation(self.robot.uid)
            self._p.resetBasePositionAndOrientation(self.robot.uid,
                                                    [robot_px, robot_py, pybullet_pos[2]],
                                                    self._p.getQuaternionFromEuler([0, 0, robot_yaw]))
        ...
        # print('actual v:', self.robot.v, 'actual w', self.robot.w)
        ...
        # for sim2real in gdc
        try:
            # randomly update humans v_pref every 3-5 seconds
            if self.config.env.scenario == 'csl_workspace' and self.config.env.csl_workspace_type == 'gdc' and self.config.humans.random_v_pref_in_episode:
                rand_period = np.random.randint(low=2, high=3)
                # print(self.global_time, rand_period)
                if round(self.global_time, 1) % rand_period == 0:
                    for human in self.humans:
                        if human.isObstacle or human.v_pref == 0:
                            continue
                        if np.random.random() <= 0.5:
                            human.v_pref = np.clip(self.config.humans.v_pref + np.random.uniform(-0.5, 0.5), 0., 1.)
        ...
        if self.config.camera.render_checkpoint is not None and self.case_counter['test']<self.config.env.test_size:
            # add another if self.case_counter['test'] - 1 == ???: to save only 1 episode
            self.render_scene()
        ...
        if self.record:
            self.episodeRecoder.wheelVelList.append([self.robot.v, self.robot.w])  # it is the calculated wheel velocity, not the measured
    ...
    def collision_checker(self):
        ...
        if self.config.action_space.kinematics == 'jackal':
            return super().collision_checker()
        # collision detection
        dmin = float('inf')
        ...
        if not collision and self.config.sim.borders:
            ...
            if self.robot.px + self.robot.radius > self.border_size or self.robot.px - self.robot.radius < -self.border_size or \
                    self.robot.py + self.robot.radius > self.border_size or self.robot.py - self.robot.radius < -self.border_size:
                collision = True
                collision_with = 'wall'
        return collision, dmin, collision_with

    def load_replay_spatial_edges(self, filepath):
        data = np.load(filepath)
        self.replay_spatial_edges = data['spatial_edges']
        self.use_replay_spatial_edges = True
        self.replay_step = 0
        print(f"[Replay] Loaded spatial edges from {filepath}, shape: {self.replay_spatial_edges.shape}")
