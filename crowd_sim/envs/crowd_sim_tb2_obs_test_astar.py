...
# Use A* planner during testing to generate waypoints, training is pure RL
class CrowdSim3DTbObsAstar(CrowdSim3DTbObsHieOM):
    def __init__(self):
        super().__init__()
    ...
    def configure(self, config):
        ...
        self.way_points_counter = config.planner.num_waypoints
    ...
    def plan_waypoints(self, flag = True):
        ...
        if path is None:
            ...
            # print("Check Path Length: ", len(path))
            if flag:
                path_resolution = np.ceil(len(path) / (self.config.planner.num_waypoints+1) * 1.0)
                self.robot_waypoint_list = path[int(path_resolution)::int(path_resolution)]
                # print("Waypoint Check: ", self.robot_waypoint_list, path_resolution)
                # print(path)
            else:
        ...
        return self.robot_waypoint_list
    ...
    def set_subgoal(self, sub_gx, sub_gy):
        self.sub_gx = sub_gx
        self.sub_gy = sub_gy
    
    def replan_subgoal(self, num_subgoal):
        # find the grid coordinate of robot (px, py) and (gx, gy)
        # print("Check Robot current position", self.robot.px, self.robot.py)
        grid_px, grid_py = self.point_to_grid(self.robot.px, self.robot.py)

        # if robot px, py is occupied, find a closest neighbor to prevent planning failure
        # swap x and y because self.om is transposed
        if self.om[grid_py, grid_px] == 1:
            grid_py, grid_px = self.find_free_neighbor(grid_py, grid_px)
        ...
        if path is None:
            ...
            if len(path) > 0:
                ...
                path = self.waypoints_to_points(path)
            # print(path)
            # print("Check Path Length: ", len(path), path)
            # sample every "path_resolution" waypoints
            # print("Check Path Length: ", len(path))
            if (len(path) < 5):
                output = []
            ...
            path_resolution = np.ceil(len(path) / (num_subgoal) * 1.0)
            self.robot_waypoint_list = path[int(path_resolution)::int(path_resolution)]
                # print("Waypoint Check: ", self.robot_waypoint_list, path_resolution)
                # print(path)
    ...
    def check_distance(self):
        robot_position = np.array([self.robot.px, self.robot.py])
        goal_position = np.array([self.robot.gx, self.robot.gy])
        subgoal_position = np.array([self.sub_gx, self.sub_gy])
        robot_to_goal_dist = np.linalg.norm(goal_position-robot_position)
        robot_to_subgoal_dist = np.linalg.norm(subgoal_position - robot_position)

        if robot_to_goal_dist < robot_to_subgoal_dist:
            return True
    ...
    def step(self, action, update=True):
        ...
        #     self.replan()
        # print("Check robot goal point in the model", self.robot.gx, self.robot.gy)
    ...
    def generate_ob(self, reset):
        ...
        # print("check ob in generate_ob: ", ob['robot_node'])
        # original: [px, py, gx, gy, theta]
        # new: [px, py, theta, subgoal1 x, subgoal1 y, ..., subgoaln x, subgoaln y, gx,
        # ob['robot_node'] = ob['robot_node'][:2] + [ob['robot_node'][4]] + self.robot_waypoint_list.flatten().tolist() + ob['robot_node'][2:4]
        if self.sub_gx is not None and self.sub_gy is not None:
            ob["robot_node"][2] = self.sub_gx
            ob['robot_node'][3] = self.sub_gy
        # print("Check robot node: -------------------------", ob["robot_node"])
        return ob
    ...
    def calc_reward(self, action, danger_zone='circle'):
        ...
        #         # visited_idx = self.fill_up_to_last_true(new_waypoint)
        #         # self.visited_waypoints[visited_idx] = True
