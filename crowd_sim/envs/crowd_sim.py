...
class CrowdSim(gym.Env):
    ...
    def configure(self, config):
        ...
        self.goal_reach_dist = config.reward.goal_reach_dist
        ...
        self.border_size = self.arena_size + self.config.sim.human_pos_noise_range
        ...
        if self.record or self.load_act:
    ...
    def generate_random_human_position(self, human_num, static_human_num=0, route_type='independent'):
        """
            ...
            if i == 0:
                ...
                if static:
                    ...
                    if self.config.env.scenario == 'csl_workspace':
                        if self.config.env.csl_workspace_type == 'hallway':
                            min_dist = 0.5 # 2.5
                        elif self.config.env.csl_workspace_type == 'gdc':
                            min_dist = 3
                        else:
                            min_dist = 2.5
                    else:
    ...
    def collision_checker(self):
        ...
        for i, human in enumerate(self.humans):
            ...
            if self.config.action_space.kinematics == 'jackal':
                if human.uid in self.cam_visible_human_uids:
    ...
    def get_human_actions(self):
        ...
        # freeze humans which are about to collide with the robot AND not in robot camera's FoV

        if self.config.action_space.kinematics == 'jackal':
            # update self.cam_human_visibility_uids, the set of humans that are visible in camera's FoV
            self.ray_test_check_human_visibility()
        ...
        for i, human in enumerate(self.humans):
            if self.humans[i].isObstacle:
                ...
                # freeze humans which are about to collide with the robot AND not in robot camera's FoV
                if self.config.action_space.kinematics == 'jackal' and self.humans[i].uid not in self.cam_visible_human_uids:
                    dx = human.px - self.robot.px
                    dy = human.py - self.robot.py
                    closest_dist = (dx ** 2 + dy ** 2) ** (1 / 2) - human.radius - self.robot.radius
                    if closest_dist < 0.4:
                ...
                if self.robot.visible and self.humans[i].react_to_robot:
                    ...
                    # Else human will always see visible robots
                        ...
                        if self.config.action_space.kinematics == 'jackal':
                            robot_state = self.robot.get_observable_state()
                            robot_state.radius *= 2
                            ob += [robot_state]
                        else:
                            ob += [self.robot.get_observable_state()]
                    else:
