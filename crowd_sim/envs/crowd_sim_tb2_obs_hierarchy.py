...
class CrowdSim3DTbObsHie(CrowdSim3DTbObs):
    ...
    def update_om(self):
        ...
        for i in range(self.grid_num):
            for j in range(self.grid_num):
                ...
                # Fill wall and everything beyond as 1 if om_boundary > border_size
                if self.om_boundary > self.border_size:
                    if (grid_x1 < -self.border_size or grid_x2 > self.border_size or
                            grid_y1 < -self.border_size or grid_y2 > self.border_size):
                        self.om[i][j] = 1
                        continue  # Skip checking obstacles for this grid cell
                ...
                for obstacle in self.cur_obstacles:
                    if obstacle[4] in self.cylinder_obs_uids:
                        ...
                        if self.check_rect_overlap(grid_x1, grid_y1, grid_x2, grid_y2,
    ...
    def plan_waypoints(self):
        ...
        if path is None:
            ...
            if len(path) > self.config.planner.num_waypoints:
                ...
                if len(self.robot_waypoint_list) < self.config.planner.num_waypoints:
                    self.robot_waypoint_list = np.vstack([self.robot_waypoint_list, path[-int(self.config.planner.num_waypoints-len(self.robot_waypoint_list))]])
        ...
        try:
            assert len(self.robot_waypoint_list) == self.config.planner.num_waypoints
        except:
            print(len(self.robot_waypoint_list))
    ...
    def calc_reward(self, action, danger_zone='circle'):
        ...
        if self.astar_succeed and self.waypoint_reward > 0:
            # add waypoint reward
            robot_position = np.array([self.robot.px, self.robot.py])
            goal_position = np.array([self.robot.gx, self.robot.gy])
            # Compute the Euclidean distances
