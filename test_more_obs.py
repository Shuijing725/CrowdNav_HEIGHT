...
def main():
	...
	parser.add_argument('--model_dir', type=str, default='data/randEnv_8to12smallobs_5to9human_0to2staticHuman_goal5to6_robCir4/ours_HH')
	...
	config = Config()
	config.sim.static_obs_num = 15
	config.sim.static_obs_num_range = 2
	config.sim.obs_min_dist = 0.5
	...
	if test_args.visualize or test_args.save_slides:
		log_file = os.path.join(test_args.model_dir, 'test', 'test_visual_more_obs.log')
	else:
		log_file = os.path.join(test_args.model_dir, 'test', 'test_'+test_args.test_model+config.env.scenario+'_more_obs.log')
