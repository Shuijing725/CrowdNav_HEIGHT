...
class OM_CNN_GRU_IL(nn.Module):
    def __init__(self, obs_space_dict, config):
        ...
        self.actor = nn.Sequential(
            init_(nn.Linear(config.SRNN.human_node_embedding_size * 2, self.output_size)), nn.Tanh(),
            init_(nn.Linear(self.output_size, self.output_size)), nn.Tanh()
        )
    ...
    def process_inputs(self, inputs, rnn_hxs, masks, infer=False):
        ...
        robot_in = reshapeT(inputs['robot_node'], seq_length, nenv)
        om_in = inputs['om'].reshape(seq_length * nenv, 1, 64, 64)
    ...
    def forward_actor(self, robot_in, om_in, rnn_hxs, masks, seq_length, nenv):
        ...
        x = self.actor(merged_features[:, :, 0, :])
        return x.view(-1, self.output_size), x, rnn_hxs
...
class OM_CNN_GRU_RL(OM_CNN_GRU_IL):
    ...
    def forward(self, inputs, rnn_hxs, masks, infer=False):
        robot_in, om_in, rnn_hxs_unmodified, masks, seq_length, nenv = self.process_inputs(inputs, rnn_hxs, masks, infer)
        hidden_actor, x, _ = self.forward_actor(robot_in, om_in, rnn_hxs_unmodified, masks, seq_length, nenv)
        hidden_critic = self.critic(x)
