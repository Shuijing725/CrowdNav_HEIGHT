...
class SpatialEdgeSelfAttn(nn.Module):
    def __init__(self, config):
        ...
        elif config.robot.policy in ['homo_transformer_obs']:
            self.input_size = config.SRNN.robot_embedding_size
        else:
            # Store required sizes
            if self.config.env.env_name in ['CrowdSimVarNum-v0']:
                self.input_size = 2
            elif self.config.env.env_name in ['CrowdSim3DTB-v0', 'CrowdSim3DTbObs-v0', 'CrowdSim3DTbObsHie-v0', 'CrowdSim3DTbObsHieOMPatch-v0','rosTurtlebot2iEnv-v0']:
